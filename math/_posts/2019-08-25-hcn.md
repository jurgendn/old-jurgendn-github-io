...
Bài viết này mình chỉ viết về vai trò của ```hình chữ nhật``` trong một vài lĩnh vực cụ thể, về chi tiết mình sẽ viết trong một bài khác.  
___
## Diện tích
...
$$\begin{aligned}
    S = \underse{n \to \infty}{lim}S_n = \underse{n \to \infty}{lim}{\sum_{i=1}^{n} f(x_i)\Delta x_{i}}
\end{aligned}$$  
Dĩ nhiên, các khoảng chia $$\Delta x_i \to 0$$ khi $$n \to \infty$$, bởi khi đó thì ta mới có thể tính được diện tích của hình trên. 
