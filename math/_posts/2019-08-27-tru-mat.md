...
[1] **Bao đóng *(Closure)* của một tập hợp**:
Cho $$\Bbb{X}$$ là một không gian $$metric$$ và tập hợp $$\Bbb{A} \subset \Bbb{X}$$. Bao đóng của $$\Bbb{A}$$ được định nghĩa như sau:  
$$\bar{A} = \{x \in \Bbb{X}|x = \underset{n \to \infty}{lim}a_n \text{  với  } a_n \in \Bbb{A}, \forall n\}$$
